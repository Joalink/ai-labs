...
CHAT_RESPONSE = {"answer": "A RAG answer", "status": "grounded", "sources": []}


@patch("app.api.v1.docs_assistant.chat.generate_response", return_value=CHAT_RESPONSE)
def test_chat_route_uses_the_browser_session(mock_generate_response):
    ...
    mock_generate_response.assert_called_once_with(
        "What is RAG?", f"session-{SESSION_ID}", None
    )
...
def test_chat_route_passes_selected_documents(mock_generate_response):
    ...
    assert response.status_code == 200
    mock_generate_response.assert_called_once_with(
        "What is RAG?", f"session-{SESSION_ID}", ["guide.pdf", "notes.pdf"]
    )
